...
- 인트로
- 메시지 처리 순서 보장해주는 kafka
- 메시지 누락과 메시지 발행 순서를 보장해주는 Transactional Outbox Pattern
- 실시간 분석에 사용되는 kafka streams + 분석용 데이터 저장하는 클라우드 서비스
- 정리
...
---
데이터베이스가 아닌 Kafka에 문제가 생겼을 경우 이벤트 발행이 되지 않을 수도 있다는 점을 지적하며, Transactional Outbox Pattern을 사용한다고 했다. 데이터 베이스 변화를 추적하는 방법인건가?
